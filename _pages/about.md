---
permalink: /
title: ""
excerpt: ""
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

{% if site.google_scholar_stats_use_cdn %}
{% assign gsDataBaseUrl = "https://cdn.jsdelivr.net/gh/" | append: site.repository | append: "@" %}
{% else %}
{% assign gsDataBaseUrl = "https://raw.githubusercontent.com/" | append: site.repository | append: "/" %}
{% endif %}
{% assign url = gsDataBaseUrl | append: "google-scholar-stats/gs_data_shieldsio.json" %}

<span class='anchor' id='about-me'></span>

<!-- Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ornare aliquet ipsum, ac tempus justo dapibus sit amet. Suspendisse condimentum, libero vel tempus mattis, risus risus vulputate libero, elementum fermentum mi neque vel nisl. Maecenas facilisis maximus dignissim. Curabitur mattis vulputate dui, tincidunt varius libero luctus eu. Mauris mauris nulla, scelerisque eget massa id, tincidunt congue felis. Sed convallis tempor ipsum rhoncus viverra. Pellentesque nulla orci, accumsan volutpat fringilla vitae, maximus sit amet tortor. Aliquam ultricies odio ut volutpat scelerisque. Donec nisl nisl, porttitor vitae pharetra quis, fringilla sed mi. Fusce pretium dolor ut aliquam consequat. Cras volutpat, tellus accumsan mattis molestie, nisl lacus tempus massa, nec malesuada tortor leo vel quam. Aliquam vel ex consectetur, vehicula leo nec, efficitur eros. Donec convallis non urna quis feugiat.

My research interest includes neural machine translation and computer vision. I have published more than 100 papers at the top international AI conferences with total <a href='https://scholar.google.com/citations?user=DhtAFkwAAAAJ'>google scholar citations <strong><span id='total_cit'>260000+</span></strong></a> (You can also use google scholar badge <a href='https://scholar.google.com/citations?user=DhtAFkwAAAAJ'><img src="https://img.shields.io/endpoint?url={{ url | url_encode }}&logo=Google%20Scholar&labelColor=f6f6f6&color=9cf&style=flat&label=citations"></a>).
 -->


Welcome to the **V**ision, **I**maging, **P**erception & **S**ecurity Laboratory (VIPS). We conduct research at the intersection of visual perception, computational imaging, and multimedia security. VIPS aims to advance next-generation multimedia perception and build secure, reliable AI systems.

### 🧠 Research Interests
- Computational Modeling of the Human Visual Perception
- Perceptual Multimedia Signal Processing and Quality Assessment
- Multimedia Quality-of-Experience
- Camera Processing Pipeline Design and Optimization

# 🧑‍🎓 Admissions

⭐⭐⭐ Our team recruit several postdocs to work on image/video quality assessment, image restoration and enhancement, multimedia scurity, 1–2 PhD students and 1–2 Master’s students each year, and I also welcome motivated bachelor students to join the group in any format.
If you are interested in joining VIPS, please feel free to reach out via [email](mailto:zhihua.wang@my.cityu.edu.hk). 


# 🔥 News
- *2026.02*: &nbsp;One paper was accepted by IEEE Transaction on Image Processing. Congratulations to Chao Huang.
- *2026.01*: &nbsp;Two papers were accepted by Pattern Recognition. Congratulations to Runhao Zeng.
<!--- *2026.01*: &nbsp;I have moved to the School of Cyberscience and Technology at SYSU, where I am excited to begin a new chapter in my academic career and research journey. -->

# 📝 Publications 

<!-- <div class='paper-box'><div class='paper-box-image'><div><div class="badge">IJCV 2026</div><img src='images/500x300.png' alt="sym" width="100%"></div></div>
<div class='paper-box-text' markdown="1">

[Deep Residual Learning for Image Recognition](https://openaccess.thecvf.com/content_cvpr_2016/papers/He_Deep_Residual_Learning_CVPR_2016_paper.pdf)

**Kaiming He**, Xiangyu Zhang, Shaoqing Ren, Jian Sun

[**Project**](https://scholar.google.com/citations?view_op=view_citation&hl=zh-CN&user=DhtAFkwAAAAJ&citation_for_view=DhtAFkwAAAAJ:ALROH1vI_8AC) <strong><span class='show_paper_citations' data='DhtAFkwAAAAJ:ALROH1vI_8AC'></span></strong>
- Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ornare aliquet ipsum, ac tempus justo dapibus sit amet. 
</div>
</div> -->
### 2026
- **Latent Fingerprint Quality Assessment for Criminal Investigations: A Benchmark Dataset and Method**<br>
  *Chao Huang, Jingxuan Zhang, Ye Zhang, Hao Wu, Peibei Cao, Zhihua Wang, Yang Yu, Xiaochun Cao*<br>
  **IEEE Transaction on Image Processing (TIP)**, 2026.<br>
  📄 [Paper]() ｜ 💻 [Code](https://github.com/wzhsysu/FIQA)

- **Test-time adaptive vision-language alignment for zero-shot group activity recognition**<br>
  *Runhao Zeng, Yirui Wang, Wenfu Peng, Xionglin Zhu, Ronghao Zhang, Zhihua Wang*<br>
  **Pattern Recognition (PR)**, 2026.<br>
  📄 [Paper](https://www.sciencedirect.com/science/article/abs/pii/S0031320325016966) ｜ 💻 [Code]()

  

### 2025
- **Sample-efficient human evaluation of large language models via maximum discrepancy competition**  
  *Kehua Feng, Keyan Ding, Kede Ma, Zhihua Wang, Qiang Zhang, Huajun Chen*  
  **Association for Computational Linguistics (ACL)**, 2025.     
  📄 [Paper](https://arxiv.org/abs/2404.08008) ｜ 💻 [Code](https://github.com/weiji-Feng/MAD-Eval)


- **Harnessing multi-modal large language models for measuring and interpreting color differences**  
  *Zhihua Wang, Yu Long, Qiuping Jiang, Chao Huang, Xiaochun Cao*  
  **IEEE Transactions on Image Processing (TIP)**, 2025.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10820056) ｜ 💻 [Code](https://github.com/LongYu-LY/CD-Reasoning)


- **Deep underwater image quality assessment with explicit degradation awareness embedding**  
  *Qiuping Jiang, Yuese Gu, Zongwei Wu, Chongyi Li, Huan Xiong, Feng Shao, Zhihua Wang*  
  **IEEE Transactions on Image Processing (TIP)**, 2025.   
  📄 [Paper](https://ieeexplore.ieee.org/document/10897305) ｜ 💻 [Code](https://github.com/yia-yuese/EDANet)



- **Multi-dimensional perceptual quality assessment for magnetic resonance images**  
  *Quankeng Huang, Yuqi Tang, Hao Li, Yang Yu, Zhihua Wang, Linfang Xiao, Wenchao Jiang, Keyan Ding*  
  **Health Information Science and Systems (HISS)**, 2025.


- **InvJND: Just Noticeable Difference Estimation via Deep Invertible Network**  
  *Qiuping Jiang, Feiyang Liu, Ziqi Wang, Zhihua Wang, Shiqi Wang, Feng Shao, Guangtao Zhai, Weisi Lin*  
  **International Journal of Computer Vision (IJCV)**, 2025.  
  📄 [Paper](#) ｜ 💻 [Code](#)


- **PyUIE: A Coarse-to-fine Deep Pyramid Network for Underwater Image Enhancement**  
  *Wenchao Jiang, Yingqing Tan, Yang Yu, Zhihua Wang, Qiuping Jiang*  
  **IEEE Transactions on Multimedia (TMM)**, 2025.  
  📄 [Paper](#) ｜ 💻 [Code](https://github.com/ttttllt/PyUIE)


- **Towards scalable and efficient full-reference omnidirectional image quality assessment**  
  *Jiebin Yan, Zhiyong Liu, Zhihua Wang, Yuming Fang, Hantao Liu*  
  **IEEE Signal Processing Letters (SPL)**, 2025.  
  📄 [Paper](https://ieeexplore.ieee.org/document/11002507/) ｜ 💻 [Code](#)


- **ICAA-Mamba: Vision Mamba for Image Color Aesthetics Assessment**  
  *Qinghao Xie, Wenchao Jiang, Zhihua Wang, Qiuping Jiang*      
  **IEEE International Conference on Acoustics, Speech, and Signal Processing (ICASSP)**, 2025.      
  📄 [Paper](https://ieeexplore.ieee.org/document/10889976) ｜ 💻 [Code](https://github.com/qinghaoya/icaa-mamba)


- **Enhancing Low-Light Images: A Synthetic Data Perspective on Practical and Generalizable Solutions**  
  *Yu Long, Qinghua Lin, Zhihua Wang, Kai Zhang, Jianguo Zhang, Yuming Fang*  
  **Association for the Advancement of Artificial Intelligence (AAAI)**, 2025.      
  📄 [Paper](https://ojs.aaai.org/index.php/AAAI/article/view/32617) ｜ 💻 [Code](https://github.com/SMBU-MM/LLIE)


- **Multi-view Evidential Learning-based Medical Image Segmentation**  
  *Chao Huang, Yushu Shi, Waikeung Wong, Chengliang Liu, Wei Wang, Zhihua Wang, Jie Wen*  
  **Association for the Advancement of Artificial Intelligence (AAAI)**, 2025.  
  📄 [Paper](https://ojs.aaai.org/index.php/AAAI/article/view/33911) ｜ 💻 [Code](https://github.com/zwx8981/NTIRE2025-XGC-Track1)

- **Multi-Dimensional Quality Assessment for UGC Videos via Modular Multi-Modal Vision-Language Models**  
  *Weixia Zhang, Bingkun Zheng, Junlin Chen, Zhihua Wang*  
  **IEEE Conference on Computer Vision and Pattern Recognition (CVPR) Workshop**, 2025.  
  📄 [Paper](https://ieeexplore.ieee.org/document/11148037) ｜ 💻 [Code](https://github.com/zwx8981/NTIRE2025-XGC-Track1)

- **Evaluating perceptual color preferences in smartphone photography: dataset and challenges**  
  *Zhihua Wang, Weixia Zhang, Wei Zhou, Xiaohong Liu, Guangtao Zhai, Patrick Le Callet*  
  **ACM Multimedia (ACM MM)**, 2025.  
  📄 [Paper](https://dl.acm.org/doi/10.1145/3746027.3758227) ｜ 💻 [Code](https://huggingface.co/datasets/zwx8981/SPCP_dataset)

- **VQualA 2025 Document Image Quality Assessment Challenge**  
  *Fan Huang et al., including Zhihua Wang*  
  **International Conference on Computer Vision (ICCV) Workshop**, 2025.

- **Vquala 2025 challenge on image super-resolution generated content quality assessment**  
  *Yixiao Li et al., including Zhihua Wang*  
  **International Conference on Computer Vision (ICCV) Workshop**, 2025.

- **Deep opinion-unaware blind image quality assessment by learning and adapting from multiple annotators**  
  *Zhihua Wang, Xuelin Liu, Jiebin Yan, Jie Wen, Wei Wang, Chao Huang*        
  **International Joint Conference on Artificial Intelligence (IJCAI)**, 2025.    
  📄 [Paper](https://www.ijcai.org/proceedings/2025/227) ｜ 💻 [Code](https://github.com/SMBU-MM/DUMBA)

### 2024
- **CD-iNet: Deep invertible network for perceptual image color difference measurement**  
  *Zhihua Wang, Keshuo Xu, Keyan Ding, Qiuping Jiang, Yifan Zuo, Zhangkai Ni, Yuming Fang*  
  **International Journal of Computer Vision (IJCV)**, 2024.  
  📄 [Paper](https://link.springer.com/article/10.1007/s11263-024-02087-7) ｜ 💻 [Code](https://github.com/hellooks/CD-iNet)

- **Dataset and Metric for Quality Assessment of HDR Tone Mapping**  
  *Qiuping Jiang, Xiwen Li, Xinyi Wang, Zhihua Wang, Guangtao Zhai*  
  **IEEE Transactions on Multimedia (TMM)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10855594) ｜ 💻 [Code]()

- **Towards Dimension-Enriched Underwater Image Quality Assessment**  
  *Qiuping Jiang, Yi Xiao, Li Ouyang, Jingchun Zhou, Zhihua Wang*  
  **IEEE Transactions on Circuits and Systems for Video Technology (TCSVT)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10689635) ｜ 💻 [Code](https://github.com/0117Tzx/MCOLE)

- **Retouched Face Image Quality Assessment Based on Differential Perception and Textual Prompt**  
  *Tianwei Zhou, Songbai Tan, Shishun Tan, Chang Tang, Zhihua Wang, Guanghui Yue*  
  **IEEE Transactions on Broadcasting (TBC)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10663283) ｜ 💻 [Code]()

- **Real-time FPGA-based closed-loop deep brain stimulation platform**  
  *Gajendra Kumar et al., including Zhihua Wang*  
  **CNS Neuroscience & Therapeutics**, 2024.

- **M2Trans: Multi-Modal Regularized Coarse-to-Fine Transformer for Ultrasound Image Super-Resolution**  
  *Zhangkai Ni et al., including Zhihua Wang*  
  **IEEE Journal Of Biomedical & Health Informatics (JBHI)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10663841) ｜ 💻 [Code](https://github.com/eezkni/M2Trans)

- **A Lesion-Fusion Neural Network for Multi-View Diabetic Retinopathy Grading**  
  *Xiaoling Luo et al., including Zhihua Wang*  
  **IEEE Journal Of Biomedical & Health Informatics (JBHI)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10488692) ｜ 💻 [Code]()

- **Rethinking and conceptualizing just noticeable difference estimation by residual learning**  
  *Qiuping Jiang et al., including Zhihua Wang*  
  **IEEE Transactions on Circuits and Systems for Video Technology (TCSVT)**, 2024.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10526290) ｜ 💻 [Code](https://github.com/Knife646/ResJND)

- **THQA: A Perceptual Quality Assessment Database for Talking Heads**  
  *Yingjie Zhou et al., including Zhihua Wang*  
  **IEEE International Conference on Image Processing (ICIP)**, 2024.  
  📄 [Paper](https://arxiv.org/abs/2404.09003) ｜ 💻 [Code](https://github.com/zyj-2000/THQA)

- **Multimodal Representation Distribution Learning for Medical Image Segmentation**  
  *Chao Huang, Weichao Cai, Qiuping Jiang, Zhihua Wang*  
  **International Joint Conference on Artificial Intelligence (IJCAI)**, 2024.  
  📄 [Paper](https://www.ijcai.org/proceedings/2024/459) ｜ 💻 [Code](https://github.com/GPIOX/Multimodal.git)

- **Multiscale Sliced Wasserstein Distances as Perceptual Color Difference Measures**  
  *Jiaqi He, Zhihua Wang, Leon Wang, Tsein-I Liu, Yuming Fang, Qilin Sun, Kede Ma*  
  **European Conference on Computer Vision (ECCV)**, 2024.  
  📄 [Paper](https://arxiv.org/abs/2407.10181) ｜ 💻 [Code](https://github.com/real-hjq/MS-SWD)

### 2023
- **Toward a blind image quality evaluator in the wild by learning beyond human opinion scores**  
  *Zhihua Wang, Zhiri Tang, Jianguo Zhang, Yuming Fang*  
  **Pattern Recognition (PR)**, 2023.  
  📄 [Paper](https://www.sciencedirect.com/science/article/abs/pii/S0031320322007750) ｜ 💻 [Code](https://github.com/SMBU-MM/OF_BIQA)

  
- **Deep blind image quality assessment powered by online hard example mining**  
  *Zhihua Wang, Qiuping Jiang, Shanshan Zhao, Wensen Feng, Weisi Lin*  
  **IEEE Transactions on Multimedia (TMM)**, 2023.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10070789) ｜ 💻 [Code](https://github.com/SMBU-MM/IQA_with_OHEM)


- **Perception-driven Deep Underwater Image Enhancement without Paired Supervision**  
  *Qiuping Jiang, Yaozu Kang, Zhihua Wang, Wenqi Ren, Chongyi Li*  
  **IEEE Transactions on Multimedia (TMM)**, 2023.  
  📄 [Paper](https://ieeexplore.ieee.org/document/10319078) ｜ 💻 [Code]()
  

- **Adaptive Structure and Texture Similarity Metric for Image Quality Assessment and Optimization**  
  *Keyan Ding, Rijin Zhong, Zhihua Wang, Yang Yu, Yuming Fang*  
  **IEEE Transactions on Multimedia (TMM)**, 2023.    
  📄 [Paper](https://ieeexplore.ieee.org/document/10323206/authors#authors) ｜ 💻 [Code](https://github.com/dingkeyan93/A-DISTS)

- **Measuring perceptual color differences of smartphone photographs**  
  *Zhihua Wang, Keshuo Xu, Yang Yang, Jianlei Dong, Shuhang Gu, Lihao Xu, Yuming Fang, Kede Ma*  
  **IEEE Transactions on Pattern Analysis and Machine Intelligence (TPAMI)**, 2023.   
  📄 [Paper](https://arxiv.org/abs/2205.13489) ｜ 💻 [Code](https://github.com/hellooks/CDNet)
  

### 2022
- **A database of visual color differences of modern smartphone photography**  
  *Keshuo Xu, Zhihua Wang, Yang Yang, Jianlei Dong, Lihao Xu, Yuming Fang, Kede Ma*  
  **International Conference on Image Processing (ICIP)**, 2022.  
  📄 [Paper](https://ieeexplore.ieee.org/document/9897498) ｜ 💻 [Code](https://github.com/hellooks/CDNet)

### 2021
- **Active fine-tuning from gMAD examples improves blind image quality assessment**  
  *Zhihua Wang, Kede Ma*  
  **IEEE Transactions on Pattern Analysis and Machine Intelligence (TPAMI)**, 2021.  
  📄 [Paper](https://ieeexplore.ieee.org/document/9399249/) 

- **Troubleshooting blind image quality models in the wild**  
  *Zhihua Wang, Haotao Wang, Tianlong Chen, Zhangyang Wang, and Kede Ma*  
  **IEEE Conference on Computer Vision and Pattern Recognition (CVPR)**, 2021.  
  📄 [Paper](https://arxiv.org/abs/2105.06747) 

### 2019
- **Towards a hybrid BCI gaming paradigm based on motor imagery and SSVEP**  
  *Zhihua Wang,Yang Yu, Ming Xu, Yadong Liu, Erwei Yin, Zongtan Zhou*  
  **International Journal of Human–Computer Interaction**, 2019.  
  📄 [Paper](https://www.tandfonline.com/doi/abs/10.1080/10447318.2018.1445068) 



# 🎖 Honors and Awards
- *2025.10* 国家级青年人才. 
- *2023.01* 深圳市鹏城特聘岗位C岗. 

# 📖 Educations
- *2018.09 - 2022.06*, Ph.D., City University of Hong Kong, Kowloon, Hong Kong. 
- *2014.09 - 2016.12*, Master of Engineering, National University of Defense Technology, Changsha, China. 
- *2010.09 - 2014.07*, Bachelor of Engineering, China University of Science and Technology, Xuzhou, China 


# 👥 Team Members
<!-- # 💬 Invited Talks
- *2021.06*, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ornare aliquet ipsum, ac tempus justo dapibus sit amet. 
- *2021.03*, Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vivamus ornare aliquet ipsum, ac tempus justo dapibus sit amet.  \| [\[video\]](https://github.com/) -->

<!-- # 💻 Working Experience
- *2022.01 - 2018.08*, [SF Technology](https://www.sf-tech.com.cn/), Shenzhen, China.
- *2017.01 - 2018.08*, [SF Technology](https://www.sf-tech.com.cn/), Shenzhen, China. -->
